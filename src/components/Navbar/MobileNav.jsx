...
const MobileNav = ({ setShowMenu, showMenu }) => {
  ...
  return (
    <div className='block md:hidden '>
      ...
      <div className={`w-full h-full fixed border-t-2 border-lime-500 px-6 py-2 z-40 top-20 left-0 transition-all duration-200 ease-linear overflow-hidden gap-8 ${showMenu ? " translate-x-0 opacity-100  " : "top-20 opacity-0 -translate-x-[100%] -z-50"} ${theme === "light" ? "bg-white" : " second"}`}>
        <div className="flex flex-col items-start gap-8 py-8">
          ...
          <div onClick={() => setShowMenu(false)} className={` flex gap-2 w-full border-b pb-2 ${pathname === "/events" ? "text-lime-500 border-lime-500" : "border-gray-400 dark:border-gray-600 text-gray-600/80"}`}>
            <MdOutlineEmojiEvents size={24} />
            <Link className="text-md  uppercase" href={"/events"}>Event</Link>
          </div>
          <div onClick={() => setShowMenu(false)} className={` flex gap-2 w-full border-b pb-2 ${pathname === "/about" ? "text-lime-500 border-lime-500" : "border-gray-400 dark:border-gray-600 text-gray-600/80"}`}>
