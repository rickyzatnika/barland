  ...
  return (
    <div className="h-screen w-full bg-gray-50 max-h-full flex items-center gap-8 flex-col justify-between sticky pt-6 left-0 top-0 z-10 ">
      ...
      <div className="w-full">
        {asideLink.map((item) => (
          <div key={item?.id} className="w-full flex flex-col  gap-1 justify-between">
            <Link href={item?.link} className={`w-full text-sm overflow-hidden relative uppercase flex items-center text-gray-600 dark:text-gray-400 gap-2 py-2.5 px-4 mb-2 transition-all ease-linear duration-100 ${pathname === item?.link ? "second text-white" : " hover:bg-[#081225]  hover:shadow-md hover:text-white "}`}>
