...
const TableListClass = () => {
  ...
  const { data, error, mutate } = useSWR(`/api/raceClasses`, fetcher);
  ...
  return (
    <>
      ...
      <div class="relative overflow-x-auto shadow-md my-14 sm:rounded-lg">
        {showModal &&
            ...
            {data?.map((list) => (
              <React.Fragment key={list?._id}>
                <tr className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                  <td className="px-6 bg-slate-200/60 py-4 uppercase font-semibold text-gray-900 whitespace-nowrap dark:text-white">
                    {list?.title}
                  </td>
                  <td className="bg-slate-200/60 px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white font-medium uppercase text-md">Harga</td>
